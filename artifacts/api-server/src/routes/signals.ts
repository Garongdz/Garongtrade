...
import { getPriceStore } from "../services/priceStore";

const router = Router();

// ── GET /api/market/top50 — served from shared WebSocket price store ──────────

router.get("/market/top50", (_req, res) => {
  const { tickers, updatedAt } = getPriceStore();
  if (tickers.length === 0) {
    res.status(503).json({ error: "Price data not yet available, try again in a few seconds" });
    return;
  }
  res.setHeader("X-Cache", updatedAt > 0 ? "HIT" : "MISS");
  res.json(tickers);
});
