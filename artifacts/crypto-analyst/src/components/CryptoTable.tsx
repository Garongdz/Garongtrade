...
function CoinRow({
  ...
  const binanceSym = coin.symbol.toUpperCase();
  ...
  return (
    <tr
      ...
      className={`border-b border-border hover:bg-muted cursor-pointer transition-colors ${flashClass}`}
    >
      ...
      <td
        className={`px-4 py-2 text-right font-mono-price text-sm transition-colors ${
      ...
      <td
        className={`px-4 py-2 text-right font-mono-price text-sm ${
          isPositive ? "text-positive" : "text-destructive"
        }`}
      >
        {isPositive ? "+" : ""}
...
export default function CryptoTable({
  ...
  if (isLoading || !prices) {
    return <div className="w-full h-96 bg-card animate-pulse" />;
  }
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-card text-muted-foreground font-medium uppercase sticky top-0 z-10 border-b border-border">
            <tr>
              ...
              <th
                className="px-4 py-2 cursor-pointer hover:text-foreground transition-colors"
                onClick={() => handleSort("name")}
              >
                <div className="flex items-center gap-1">
                  Pair <ArrowUpDown className="h-3 w-3" />
                </div>
              </th>
              <th
                className="px-4 py-2 text-right cursor-pointer hover:text-foreground transition-colors"
              ...
              <th
                className="px-4 py-2 text-right hidden md:table-cell cursor-pointer hover:text-foreground transition-colors"
              ...
              <th
                className="px-4 py-2 text-right hidden lg:table-cell cursor-pointer hover:text-foreground transition-colors"
              ...
              <th className="px-4 py-2 text-right hidden xl:table-cell text-muted-foreground">
                Live Chart
              </th>
            </tr>
          </thead>
